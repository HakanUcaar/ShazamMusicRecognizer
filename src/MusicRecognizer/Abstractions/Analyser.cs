...
{
	public static void ReadSamples(this Analyser analyser, float[] samples, int count)
	{
		// Convert float samples to the format your Analyser expects
		// This depends on your Analyser implementation
		// You might need to convert to short[] or feed directly as float[]

		var shortSamples = new short[count];
		for (int i = 0; i < count; i++)
		{
			shortSamples[i] = (short)(samples[i] * 32767);
		}

		// Feed to your analyser - adjust this call based on your Analyser's API
		// analyser.FeedSamples(shortSamples);
	}
}
